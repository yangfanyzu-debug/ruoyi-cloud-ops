...
public class SysUserController extends BaseController
{
     ...
     * Skill IDE 审核人候选列表
     */
    @GetMapping("/skill-ide-reviewers")
    public AjaxResult skillIdeReviewers()
    {
        SysUser query = new SysUser();
        query.setStatus("0");
        List<SysUser> list = userService.selectUserList(query);
        List<Map<String, Object>> items = list.stream().map(user -> {
            Map<String, Object> item = new HashMap<>();
            item.put("id", user.getUserId());
            item.put("userId", user.getUserId());
            item.put("username", user.getUserName());
            item.put("userName", user.getUserName());
            item.put("displayName", user.getNickName());
            item.put("nickName", user.getNickName());
            item.put("status", user.getStatus());
            item.put("email", user.getEmail());
            item.put("role", SecurityUtils.isAdmin(user.getUserId()) ? "admin" : "user");
            return item;
