...
assert.match(reportTable, /aria-label="预览最新版本"[\s\S]*?<i class="el-icon-view"/, 'preview action must render a visible icon')
assert.match(reportTable, /aria-label="下载最新版本"[\s\S]*?<i class="el-icon-download"/, 'download action must render a visible icon')
